class Timer {
  ...
  void setByString(String stringIn) {
    String[] s = stringIn.split(" ");    
    set(Integer.valueOf(s[0]) * PERDAY + 
        Integer.valueOf(s[1]) * PERHOUR +
        Integer.valueOf(s[2]) * PERMIN + 
        Integer.valueOf(s[3]) * PERSEC + 
        Integer.valueOf(s[4]) * PERHUN
       );
  }
}
